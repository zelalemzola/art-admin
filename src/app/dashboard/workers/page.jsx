...
const Workers = () => {
  ...
  const validateForm = (formState) => {
    ...
    if (!formState.age) newErrors.push('age is required');
    if (!formState.height) newErrors.push('height is required');
    if (!formState.weight) newErrors.push('height is required');
    if (!formState.address) newErrors.push('address is required');
    if (!formState.phoneNumber) newErrors.push('phoneNumber is required');
    if (!formState.educationalLevel) newErrors.push('educational Level is required');
    ...
    if (formState.skills.some(skills => !skills)) newErrors.push('All skills fields must be filled');
    return newErrors;
  };
  ...
  const handleSubmit = async (event, isUpdate = false) => {
    ...
    const { id, name, fathersName, grandFathersName, imageUrl, imageKey, pricePerMonth,pricePerHour, experience, review, category, documentUrl, documentKey, documentName, isAvailable, languages,age,address,height,weight,phoneNumber,skills,educationalLevel } = formState;

    if (isUpdate) {
      await axios.put(`/api/maids/${id}`, { name, fathersName, grandFathersName, imageUrl, imageKey, pricePerMonth,pricePerHour, experience, review, category, documentUrl, documentKey, documentName, isAvailable, languages,age,address,height,weight,phoneNumber,skills,educationalLevel });
    } else {
      await axios.post('/api/maids', { name, fathersName, grandFathersName, imageUrl, imageKey, pricePerMonth,pricePerHour, experience, review, category, documentUrl, documentKey, documentName, isAvailable, languages,age,address,height,weight,phoneNumber,skills,educationalLevel });
    }
  ...
  const handleEdit = (maid) => {
    setUpdateFormState({
      ...
      educationalLevel:maid.educationalLevel,
      skills: maid.skills,
    });
  ...
  const handleChange = (e, isUpdate = false) => {
    ...
    } else if (name.startsWith('experience') || name.startsWith('review') || name.startsWith('languages') || name.startsWith('skills')) {
              ...
              <div className="flex flex-col items-start gap-3">
                <ScrollArea className='h-full  w-full bg-white px-4 '>
                  <div className="flex flex-col gap-3 h-[360px] text-black pt-2 py-2 px-2 w-full">
                    ...
                    <div className="flex items-center gap-2">
                      <h2 className='text-lg font-bold text-primary'>Age</h2>
                      <Input name="age" type="number" value={createFormState.age} onChange={(e) => handleChange(e)} placeholder="Enter Age" required className='w-1/4' />
                    </div>
                    <div className="flex items-center gap-2">
                      <h2 className='text-lg font-bold text-primary'>Weight</h2>
                      <Input name="weight" type="number" value={createFormState.weight} onChange={(e) => handleChange(e)} placeholder="Enter Weight" required className='w-1/4' />
                    </div>
                    <div className="flex items-center gap-2">
                      <h2 className='text-lg font-bold text-primary'>Height</h2>
                      <Input name="height" type="number" value={createFormState.height} onChange={(e) => handleChange(e)} placeholder="Enter Height" required className='w-1/4' />
                    </div>
                    <div className="flex items-center gap-2">
                      <h2 className='text-lg font-bold text-primary'>Phone Number</h2>
                      <Input name="phoneNumber" type="number" value={createFormState.phoneNumber} onChange={(e) => handleChange(e)} placeholder="Enter Phone Number" required className='w-1/4' />
                    </div>
                    <div className="flex items-center gap-2">
                      <h2 className='text-lg font-bold text-primary'>Phone Number</h2>
                      <Input className='w-[32%]' name="address" value={createFormState.address} onChange={(e) => handleChange(e)} placeholder="Enter Address" required />
                    </div>
                    <div className="flex items-center gap-2">
                      <h2 className='text-lg font-bold text-primary'>Educational Level</h2>
                      <Input className='w-[32%]' name="educationalLevel" value={createFormState.educationalLevel} onChange={(e) => handleChange(e)} placeholder="Enter Educational Level" required />
                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                      <h2 className='text-lg font-bold text-primary'>Enter workers skills </h2>
                      {createFormState.skills.map((skill, index) => (
                        <div key={index} className='flex items-center gap-3'>
                          <Input name={`skills-${index}`} value={skill} onChange={(e) => handleChange(e)} placeholder="Drawing" className='w-1/3' />
                          <Button type="button" onClick={() => handleRemoveField('skills', index)}>Remove</Button>
                        </div>
                      ))}
                      <Button type="button" onClick={() => handleAddField('skills')}>Add Skills</Button>
                    </div>
                    <div className="flex flex-col gap-3">
                          ...
                          <div className="flex flex-col items-start gap-3">
                            <ScrollArea className='h-full  w-full bg-white px-4 '>
                              <div className="flex flex-col gap-3 h-[360px] text-black pt-2 py-2 px-2 w-full">
                                ...
                                <div className="flex items-center gap-3">
                                  <h2 className='text-lg text-primary font-bold'>Update Age </h2>
                                  <Input name="age" type="number" value={updateFormState.age} onChange={(e) => handleChange(e, true)} placeholder="Enter age" required className='w-[30%]'/>
                                </div>
                                <div className="flex items-center gap-3">
                                  <h2 className='text-lg text-primary font-bold'>Update Weight </h2>
                                  <Input name="weight" type="number" value={updateFormState.weight} onChange={(e) => handleChange(e, true)} placeholder="Enter weight" required className='w-[30%]'/>
                                </div>
                                <div className="flex items-center gap-3">
                                  <h2 className='text-lg text-primary font-bold'>Update Height </h2>
                                  <Input name="height" type="number" value={updateFormState.height} onChange={(e) => handleChange(e, true)} placeholder="Enter height" required className='w-[30%]'/>
                                </div>
                                <div className="flex items-center gap-3">
                                  <h2 className='text-lg text-primary font-bold'>Update Phone Number </h2>
                                  <Input name="phoneNumber" type="number" value={updateFormState.phoneNumber} onChange={(e) => handleChange(e, true)} placeholder="Enter Phone Number" required className='w-[30%]'/>
                                </div>
                                <div className="flex items-center gap-3">
                                  <h2 className='text-lg text-primary font-bold'>Update Address </h2>
                                  <Input className='w-[32%]' name="address" value={updateFormState.address} onChange={(e) => handleChange(e, true)} placeholder="Enter Address" required />
                                </div>
                                <div className="flex items-center gap-3">
                                  <h2 className='text-lg text-primary font-bold'>Update Educational Level </h2>
                                  <Input className='w-[32%]' name="educationalLevel" value={updateFormState.educationalLevel} onChange={(e) => handleChange(e, true)} placeholder="Educational Level" required />
                                </div>
                                <div className="flex flex-col gap-3">
                                  <h2 className="text-primary text-lg font-bold">Update Skills</h2>
                                  {updateFormState.skills.map((exp, index) => (
                                    <div key={index} className='flex items-center gap-3'>
                                      <Input name={`skills-${index}`} value={exp} onChange={(e) => handleChange(e, true)} placeholder="Drawing" className='w-[20%]'/>
                                      <Button type="button" onClick={() => handleRemoveField('skills', index, true)}>Remove</Button>
                                    </div>
                                  ))}
                                  <Button type="button" onClick={() => handleAddField('skills', true)}>Add Experience</Button>
                                </div>
                                <div className="flex flex-col gap-3">
